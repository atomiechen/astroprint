...
- `src/components/PreviewShell.astro` is the theme-neutral preview shell with navigation, print button, preview status, scroll restoration, and normal/Paged.js preview branching. Its default Home link should use Astro's `BASE_URL` so subpath deployments stay inside the configured base.

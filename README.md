...
For a completely custom template, create your own layout and stylesheet, then use the directive classes generated by `aprint` (`.aprint-entry`, `.aprint-two-column-list`, and so on), or extend the directive mapping with the integration `directives` option.
